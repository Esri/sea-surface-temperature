...
class GlobeSpinner extends HTMLElement {
  ...
  constructor({
                view,
                scale = GlobeSpinner.SCALE_OPTION.MEDIUM,
                target = GlobeSpinner.TARGET.CENTER,
                direction = GlobeSpinner.DIRECTION.NONE,
                speed = GlobeSpinner.SPEED.NORMAL,
                configurable = true,
                disabled = false
              }) {
    super();
  ...
  setSpinState(direction) {
    ...
    this.dispatchEvent(new CustomEvent('state-change', {detail: {direction: this.#spinDirection}}));
  }
